...
impl PtyIoActorRunner {
    ...
    /// Milliseconds the run loop may block before a pending submission
    /// boundary (`enter` write) is due. `None` when no submission is
    /// waiting on a deadline.
    fn submission_poll_timeout_ms(&self) -> Option<i32> {
        let Some(ActiveSubmission {
            phase: SubmissionPhase::WaitingUntil(deadline),
            ..
        }) = self.active_submission.as_ref()
    ...
    fn nudge_poll_timeout_ms(&self) -> Option<i32> {
        let due = self.nudge_restore_due?;
    ...
    /// Milliseconds the run loop may block before either a pending

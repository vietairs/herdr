...
impl PaneRuntimeIo {
    ...
    fn write_terminal_response(&self, response: impl FnOnce() -> Option<Bytes>) {
        match self {
            ...
            // Host-appearance/OSC query responses are meaningful only to a
            // local child sharing this process's PTY; a remote-backed pane's
            // child is owned by the remote server, which handles its own
            // host-terminal responses independently.
            PaneRuntimeIo::Remote(_) => {}
            #[cfg(test)]
...
impl PaneRuntime {
    ...
    pub(crate) fn spawn_remote(
        ...
        let io = {
            ...
            let on_read = Box::new(move |bytes: &[u8]| {
                ...
                // A remote pane's BEL is the remote program asking for
                // attention; publish it on the same path a local pane uses so
                // the client rings for federated panes too.
    ...
    pub fn cwd(&self) -> Option<std::path::PathBuf> {
        // `reported_cwd` is only ever populated via `publish_reported_cwd`,
        // which already validates the path (absolute + existing directory)
        // before admitting it to the cache. Re-validating here would mean a
        // directory that is briefly unavailable (e.g. a transient stat
        // failure, or removed after a cd away) silently evicts an otherwise
        // still-accurate cached report.
    ...
    async fn queue_user_input_submission_delivers_delayed_enter_to_a_remote_pane_input_channel() {
        ...
        assert!(matches!(
            ...
            FederationMessage::Terminal(TerminalChannelMessage::Input {
                ...
                mount_generation: 7,
                bytes,
            }) if terminal_id == "term_1" && bytes == b"\r"
        ));
    }
}

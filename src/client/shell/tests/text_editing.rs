    ...
    for field in 0..5 {
        for empty in [false, true] {
            ...
            *editor(&mut state) = TextEditor::from(if empty { "  " } else { "  ab " });
            ...
            // Emptying the box means different things to the two rename
            // targets. A tab (field 3) that is still auto-named has no
            ...
            // back would pin a derived string as a real override.
            if empty && matches!(field, 3 | 4) {
                assert!(result.actions.is_empty());
                continue;
            }
            let [ClientShellAction::Endpoint { request, .. }] = &result.actions[..] else {

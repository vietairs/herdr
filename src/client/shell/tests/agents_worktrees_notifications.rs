...
fn muted_agent_sidebar_rows_do_not_stack_terminal_faint() {
    ...
    // The agent sidebar gates the tab label on `name_source`, not on the
    // legacy `custom_label` flag, so an upstream-shaped "this tab was
    // renamed" setup has to say so through the naming ladder too.
    projected.tabs[0].name_source = crate::workspace::naming::NameSource::Override;

...
/// Windows-side path-shape recognition. The Unix drop path uses
/// `crate::image_path::local_image_path_from_text`, which folds these two
/// steps together; `image_path` is `cfg(unix)`, so the Windows client-event
/// bridge keeps its own copy here.
#[cfg(any(windows, test))]

...
impl<'a> RowCellIter<'a> {
    ...
    pub fn grapheme_text_into(
        ...
        // Avoid GRAPHEMES_UTF8 here. The underreporting of multi-codepoint
        // graphemes by its required-byte query was observed against the
        // libghostty-vt 0.6.7 vendor and has NOT been re-tested against the
        // current vendored base. Upstream uses GRAPHEMES_UTF8 on this same
        // base, so this codepoint path may now be redundant; confirm with
        // grapheme_cluster_mode_renders_zwj_family_in_single_wide_cell before
        // dropping it.

...
# remote_image_paste = "ctrl+v" # active on a pane of a mounted remote workspace and in herdr --remote; empty disables clipboard image paste entirely
# new_tab = "prefix+c"
...
fn main() -> io::Result<()> {
    ...
    let (args, remote_launch) = match remote::extract_federated_remote_args(&args) {
        Ok(parsed) => parsed,
        Err(err) => {
            eprintln!("error: {err}");
            eprintln!("run 'herdr --help' for usage");

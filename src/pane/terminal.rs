...
impl GhosttyPaneTerminal {
    ...
    fn write_pty_bytes_with_ordered_responses(
        ...
        events.extend(
            c1_xtgettcap_responses
                .into_iter()
                .map(OrderedPtyResponseEvent::C1),
        );
        ...
        for event in events {
            ...
            if matches!(&event, OrderedPtyResponseEvent::C1(response) if response.suppress_native) {
            ...
            match event {
                ...
                OrderedPtyResponseEvent::C1(response) => {
...
enum OrderedPtyResponseEvent {
    ...
    /// Legacy raw-C1 XTGETTCAP reply; ordinary XTGETTCAP remains native.
...
mod tests {
    ...
    fn xtwinops_size_queries_follow_successful_resize() {
        ...
        let pane_id = PaneId::from_raw(1);
        pane.resize(24, 80, 9, 18, false);
        pane.resize(30, 100, 10, 20, false);
    ...
    fn xtwinops_size_queries_stay_silent_without_pixel_geometry() {
        ...
        for (cell_width_px, cell_height_px) in [(0, 0), (0, 18), (9, 0)] {
            pane.resize(24, 80, cell_width_px, cell_height_px, false);
            let result = pane.process_pty_bytes(pane_id, 0, b"\x1b[14t\x1b[16t\x1b[18t", &tx);
            assert!(result.terminal_responses.is_empty());
        }
    }
    ...
    fn enabling_in_band_size_reports_after_alt_screen_resize_reports_current_size() {
        ...
        assert!(pane.resize(24, 92, 9, 18, false).is_empty());

...
/// documents OSC 9;9 for the same practical purpose. This is a fallback for
/// forms `Terminal::take_pwd_changes` (OSC 7 only, via libghostty-vt's native
/// `GHOSTTY_TERMINAL_OPT_PWD_CHANGED` callback) does not cover.
#[derive(Debug, Default)]
...
impl CwdOscTracker {
    pub(super) fn observe(&mut self, bytes: &[u8]) {
        ...
        collector.observe(bytes, |body| {
            if let Some(cwd) = parse_cwd_osc(body) {
                pending.push(cwd);
            }
        });
    }

...
fn test_headless_server_with_event_hub(event_hub: api::EventHub) -> HeadlessServer {
    ...
    #[cfg(unix)]
    let (federation_listener, federation_socket_path, federation_socket_identity) = {
        let federation_path = federation_socket_path(&socket_path);
        prepare_socket_path(&federation_path).expect("prepare test federation socket path");
        ...
        restrict_socket_permissions(&federation_path)
            .expect("restrict test federation socket permissions");

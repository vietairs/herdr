...
fn reattach_after_detach_shows_current_state() {
    ...
    let (version, error) =
        client_handshake(&mut stream_a, 6, 80, 24).expect("handshake should succeed");
    assert_eq!(version, 6);
    assert!(error.is_none(), "{:?}", error);
    ...
    let (version, error) =
        client_handshake(&mut stream_b, 6, 80, 24).expect("handshake should succeed");
    assert_eq!(version, 6);
    assert!(
        error.is_none(),
...
fn processes_survive_during_and_after_detach() {
    ...
    let (version, error) =
        client_handshake(&mut stream_b, 6, 80, 24).expect("reattach handshake should succeed");
    assert_eq!(version, 6);
    assert!(error.is_none(), "{:?}", error);
...
fn detached_output_preserves_last_attached_pty_size() {
    ...
    let (version, error) =
        client_handshake(&mut stream, 6, 120, 40).expect("handshake should succeed");
    assert_eq!(version, 6);
    assert!(error.is_none(), "{:?}", error);
    drain_messages(&mut stream);

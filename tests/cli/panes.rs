...
fn wait_output_matches_recent_unwrapped_text() {
    ...
    let run = run_cli(
        &socket_path,
        &["pane", "run", &pane_id, &format!("sh {}", script.display())],
    );
...
fn closing_workspace_terminates_processes_inside_it() {
    ...
    let ran = run_cli(&socket_path, &["pane", "run", &root_pane_id, &command]);
    assert!(
        ran.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&ran.stderr)
    ...
    let closed = run_cli(&socket_path, &["workspace", "close", &workspace_id]);
    assert!(
        closed.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&closed.stderr)

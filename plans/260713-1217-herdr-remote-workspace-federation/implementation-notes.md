...
**Evidence:** `runtimeId` seen locally on VM100 == `runtimeId` returned remotely via pairing env -> remote traffic lands on the new 1.4.143 process. `nc 192.0.2.161:45511` succeeds from Mac.
...
correctly showed the federated workspace (`192.0.2.163#fedsmoke`), but the selected pane showed a live,
...
(`-F /tmp/herdr-ssh-831358-0/config -S .../ctl 192.0.2.163 hostname`) and got `bio-1-ubuntu` back — i.e.
...
192.0.2.163 hostname` (via the fedsmoke process's own tunnel) -&gt; `bio-1-ubuntu`; `ps aux` before/after

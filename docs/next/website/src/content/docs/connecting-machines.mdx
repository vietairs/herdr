...
Multi-machine connections are supported on Linux, macOS, and Windows clients, connecting to Linux and macOS servers on x86_64 or aarch64. On Windows the connection is not yet verified against a live host, and OpenSSH there has no connection multiplexing, so key or agent authentication is effectively required. Native Windows servers are not supported as SSH targets. See [Remote attach over SSH](/docs/persistence-remote/#remote-attach-over-ssh) for SSH configuration, authentication, and custom binaries.
...
Renaming a mounted remote workspace or tab is local-only: it changes what you see, but it is never sent to the serving host, is not saved, and is lost the next time that workspace or tab is remounted. It also does not affect the name the workspace or tab owner sees on the serving host itself.

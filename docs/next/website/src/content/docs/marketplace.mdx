...
Add the GitHub topic `herdr-plugin` to your repository. That topic is the exact
signal the future index will use for discovery, so tagging now means your plugin
is indexed on day one, with no migration and no resubmission.

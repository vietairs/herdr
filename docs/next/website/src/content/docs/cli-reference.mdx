...
`agent read` reads the resolved terminal stream. `agent prompt` atomically submits text and Enter, honoring the pane's live bracketed-paste mode; add `--wait` to wait for the first settled `idle`, `done`, or `blocked` state afterward. `agent send-keys` sends logical keys (like `esc` or `ctrl+c`) for interactive agent UI controls. `agent get`, `agent focus`, `agent wait`, and `agent attach` require the resolved terminal to have agent identity. `agent rename` can assign that identity.

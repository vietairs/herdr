...
For a federated (remote-mounted) workspace, `workspace close` only removes your local mirror; the copy on the serving host keeps running. `workspace close-remote` asks the serving host to close its own workspace instead. It cannot resolve synchronously — the request goes out over the federation link and the host answers later — so it returns a `remote_close_pending` error immediately (the request was accepted and sent, not a failure) and your local mirror disappears once the host confirms the close. It returns `remote_close_unsupported` for a non-federated workspace, when the mount has no live link, or when the serving host never agreed to the close capability.
...
Same distinction as `workspace close` / `workspace close-remote`: for a federated tab, `tab close` only removes your local mirror tab, leaving the host's copy running, while `tab close-remote` asks the serving host to close its own tab. It answers immediately with `remote_close_pending` (accepted and sent, not a failure) and the local mirror tab disappears once the host confirms; it returns `remote_close_unsupported` for a non-federated tab, a dead mount, or a peer that never agreed to the close capability.

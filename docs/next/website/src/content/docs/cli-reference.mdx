...
herdr workspace close-remote <workspace_id>
```
...
`workspace create` without `--cwd` follows the session's current workspace, and that includes federation: when the workspace the create starts from is a mounted remote workspace, the new workspace is created **on that serving host**, not locally — the same way splitting a pane inside a federated workspace splits on the remote. The create starts from the sidebar-selected workspace in navigate mode, otherwise the active workspace. Because the serving host answers asynchronously, the response is `workspace_create_requested` with the `origin` mount instead of `workspace_created`: it carries no workspace, tab, or pane IDs, and the new workspace appears locally a moment later through the mount's normal resync. Scripts that need a local workspace, or need IDs back in the same response, must pass `--cwd`; an explicit path is always taken as a deliberate local-directory choice and never redirected. If the mount is not connected, the command fails with `remote_workspace_create_unsupported` rather than quietly creating a local workspace.

For a federated (remote-mounted) workspace, `workspace close` only removes your local mirror; the copy on the serving host keeps running. `workspace close-remote` asks the serving host to close its own workspace instead. It cannot report the finished close — the request goes out over the federation link and the host answers later — so it succeeds immediately with a `workspace_close_requested` result naming the mount the request went out over, exits 0, and your local mirror disappears once the host confirms the close. It returns `remote_close_unsupported` for a non-federated workspace, when the mount has no live link, or when the serving host never agreed to the close capability.
...
herdr tab close-remote <tab_id>
```

A tab is another terminal layout inside a workspace. Without `--workspace`, `tab create` uses the active workspace and fails if none exists. Its JSON response exposes `.result.tab.tab_id` and `.result.root_pane.pane_id`. When the target workspace is a live remote mount the create is forwarded to the serving host instead, and the response is `tab_create_requested` carrying only `.result.origin` — no `.result.tab`, no `.result.root_pane` — with the tab appearing through the mount's normal resync; `--cwd` then fails with `remote_tab_cwd_unsupported` and `--env` with `remote_tab_env_unsupported`, because both name things on the calling machine. Closing a workspace's last tab also closes the workspace, matching the TUI close-tab action. If `confirm_close` is enabled and closing the tab would also close a whole worktree group, `tab close` returns a `confirmation_required` error instead.

Same distinction as `workspace close` / `workspace close-remote`: for a federated tab, `tab close` only removes your local mirror tab, leaving the host's copy running, while `tab close-remote` asks the serving host to close its own tab. It succeeds immediately with a `tab_close_requested` result naming the mount the request went out over (exit 0) and the local mirror tab disappears once the host confirms; it returns `remote_close_unsupported` for a non-federated tab, a dead mount, or a peer that never agreed to the close capability.

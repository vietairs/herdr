...
`tab.create` 本身即支持联邦,不存在单独的 `tab.create_remote` 方法。当目标工作区(显式的 `workspace_id`,否则为当前活动工作区)属于一个仍在连接的远程挂载时,标签页会在提供服务的主机上创建,而不是在本地创建;这适用于包括 CLI 和脚本在内的所有调用方。此时响应是携带 `origin` 挂载键的 `tab_create_requested`,而不是 `tab_created`:因为标签页尚不存在,它不包含标签页 id 或窗格 id;它会通过该挂载的常规重新同步在本地出现,就像远程用户自己创建的标签页一样。依据 `tab_created` 分支的脚本调用方必须处理这种形态。`label` 会发送到提供服务的主机,并在进入该主机自身界面之前完成净化和长度截断。显式的 `cwd` 会以 `remote_tab_cwd_unsupported` 失败,非空的 `env` 会以 `remote_tab_env_unsupported` 失败:两者都是本地机器的概念,在提供服务的主机上没有意义,因此会被明确拒绝而不是静默忽略。当目标工作区的挂载没有可用链路时,请求会以 `remote_tab_create_unsupported` 失败,而不会静默创建本地标签页。如果提供服务的主机本身会把该创建重定向到它所挂载的主机,它会以 `tab_create_redirected` 拒绝,而不是报告一个调用方无法访问的标签页。

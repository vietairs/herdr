...
`agent read` 读取解析出的终端流。`agent prompt` 原子化地发送文本和回车,遵循窗格当前的括号粘贴模式;加 `--wait` 可以等待发送后首次进入 `idle`、`done` 或 `blocked` 的稳定状态。`agent send-keys` 发送逻辑按键(如 `esc` 或 `ctrl+c`)用于交互式智能体 UI 控制。`agent get`、`agent focus`、`agent wait` 和 `agent attach` 要求解析出的终端具有智能体身份。`agent rename` 可以赋予这个身份。

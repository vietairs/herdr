...
`[[startup]]` 命令会在 Herdr 恢复会话、其 API socket 就绪之后,为每个已启用的
插件运行一次。它们在实时交接(live handoff)期间新服务器接管时会再次运行,
但客户端连接、配置重新加载或插件被链接/启用时不会运行。Herdr 会异步启动
它们,并把完成情况记录在正常的插件命令日志中。启动失败不会导致服务器停止。

启动钩子是一次性的初始化命令,而不是受监管的守护进程。钩子应当恢复插件
拥有的状态、调用任何所需的 Herdr API,然后退出。例如,插件可以将一个声明式
的 Agent 视图保存到 `HERDR_PLUGIN_STATE_DIR` 下,然后在其启动钩子中读取并
重新应用该视图。

启动钩子会收到普通的运行时插件环境变量,以及 `HERDR_PLUGIN_EVENT=startup`。
安装预览会列出每个启动命令,以便用户审查即将自动运行的代码。

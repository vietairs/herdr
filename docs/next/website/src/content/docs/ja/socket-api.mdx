...
`tab.create` は、別に `tab.create_remote` を設けるのではなく、それ自体がフェデレーション対応です。対象ワークスペース (明示された `workspace_id`、省略時はアクティブなワークスペース) がライブなリモートマウントに属している場合、タブはローカルではなくサービング側ホストで作成されます。これは CLI やスクリプトを含むすべての呼び出し元に適用されます。その場合のレスポンスは `tab_created` ではなく、`origin` マウントキーを持つ `tab_create_requested` になります。タブはまだ存在しないためタブ id もペイン id も含まれず、リモートのユーザーが作成したタブとまったく同じように、そのマウント通常の再同期を通じてローカルに現れます。`tab_created` で分岐するスクリプトは、この形を扱う必要があります。`label` はサービング側ホストへ送られ、そのホストの UI に表示される前にサニタイズと長さのクランプが行われます。明示的な `cwd` は `remote_tab_cwd_unsupported`、空でない `env` は `remote_tab_env_unsupported` で失敗します。どちらもローカルマシン固有の概念でサービング側ホストでは意味を持たないため、黙って無視せず明示的に拒否します。対象ワークスペースのマウントにライブなリンクがない場合、リクエストは `remote_tab_create_unsupported` で失敗し、ローカルタブが黙って作成されることはありません。`tab_create_requested` が返された後、サービング側ホスト自身の応答は非同期に届き、API や CLI の呼び出し元には報告されません。拒否 (サービング側ホストがさらに自分のマウント先へ作成をリダイレクトする場合の `tab_create_redirected`、`not the mounted controller`、`<code>: tab could not be created on the remote host` を含む) は TUI のトーストとサーバーログにのみ現れます。スクリプトからの呼び出し元は、2 つ目のレスポンスを期待するのではなく、再同期やイベントでタブが現れるのを待つ必要があります。

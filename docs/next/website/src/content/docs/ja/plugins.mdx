...
`[[startup]]` コマンドは、Herdr がセッションを復元し API ソケットの準備が
整った後、有効な各プラグインごとに一度だけ実行されます。ライブハンドオフで
新しいサーバーが引き継いだ際にも再度実行されますが、クライアントがアタッチ
したとき、設定がリロードされたとき、プラグインがリンクまたは有効化された
ときには実行されません。Herdr はこれらを非同期に開始し、通常のプラグイン
コマンドログに完了を記録します。起動フックの失敗はサーバーを停止させません。

起動フックは、監視され続けるデーモンではなく、一度きりの初期化コマンドです。
フックはプラグイン所有の状態を復元し、必要な Herdr API を呼び出してから
終了するべきです。例えば、プラグインは `HERDR_PLUGIN_STATE_DIR` の下に
宣言的な Agent ビューを保存しておき、起動フックからそのビューを読み込んで
再適用できます。

起動フックは通常のランタイムプラグイン環境と `HERDR_PLUGIN_EVENT=startup`
を受け取ります。インストールプレビューにはすべての起動コマンドが列挙され、
ユーザーは自動的に実行されるコードを事前に確認できます。

...
`agent read` は解決されたターミナルストリームを読みます。`agent prompt` はテキストと Enter をペインのライブなブラケットペーストモードを尊重しつつ一括送信します。送信後に `idle`・`done`・`blocked` のいずれかの状態に落ち着くのを待つには `--wait` を付けます。`agent send-keys` は `esc` や `ctrl+c` のような論理キーをインタラクティブなエージェント UI の操作に送ります。`agent get`、`agent focus`、`agent wait`、`agent attach` は、解決されたターミナルがエージェントのアイデンティティを持つことを要求します。`agent rename` はそのアイデンティティを割り当てられます。

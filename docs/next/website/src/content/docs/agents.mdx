...
Use workspace, tab, pane, and layout commands to create topology first. Then activate an existing shell pane as a supported interactive agent with a unique name:
...
`agent start` never creates or splits topology. Herdr chooses the canonical executable for the requested kind, labels the target pane with the unique name, and returns after the expected interactive agent is detected as idle, done, or blocked. Optional native arguments follow `--`:

```bash
herdr agent start reviewer --kind pi --pane w1:p2 -- --name scratch --no-session
```

Submit a prompt by name. `--wait` binds the wait to that prompt and returns when the agent finishes or becomes blocked:

```bash
herdr agent prompt reviewer "Review the current diff" --wait --timeout 120000
```

`agent prompt` honors the terminal's live bracketed-paste mode before sending Enter, so text such as `A != B` reaches agents without triggering input-mode keybindings. Use it for submissions; `agent send` remains a low-level literal-text operation and does not submit a turn.

For an agent that is already working, `agent wait` returns when its status becomes idle, done, or blocked. It returns immediately if one of those statuses is already current.
...
Use `herdr pane ...` commands for ordinary terminals, servers, tests, shells, topology, and low-level terminal input.

...
- The dialog closes as soon as the request is accepted, not when the mount finishes. Preparing and
  connecting to the remote host runs in the background with no progress indicator and no cancel. A
  successful mount appears as new workspaces in the sidebar; targets that fail validation (empty
  input, an option-like token, `localhost`) are rejected immediately and shown inline in the
  dialog, while a target that dials and fails later reports through your configured notification
  channel (`ui.toast.delivery`).

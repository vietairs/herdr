...
Ratio-only layout changes (`layout.balance`, `layout.set_split_ratio`, and
the auto-resize-splits behavior) do not trigger a federation resync, so a
mounted remote workspace's local mirror can show stale ratios until an
unrelated structural event (a pane or tab created, closed, or moved) forces
a resync.

...
`workspace.close` and `tab.close` only ever remove your local mirror of a federated (remote-mounted) workspace or tab; they never reach the federation link, so the copy on the serving host keeps running. `workspace.close_remote` and `tab.close_remote` are the distinct opt-in verbs that ask the serving host to close its own workspace or tab. Because that answer arrives asynchronously over the federation link, they cannot resolve synchronously like the plain verbs: they return an error envelope with code `remote_close_pending` once the request has been sent (the close was accepted, not refused), and the local mirror disappears once the host confirms it closed. They return `remote_close_unsupported` when the target is not federated, its mount has no live link, or the peer never agreed to the close capability.

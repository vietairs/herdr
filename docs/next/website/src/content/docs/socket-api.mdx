...
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.close`, `workspace.close_remote` |
| Worktree | `worktree.list`, `worktree.create`, `worktree.open`, `worktree.remove` |
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.move`, `tab.close`, `tab.close_remote` |
...
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio`, `layout.balance` |
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.prompt`, `agent.wait`, `agent.send_keys`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.set`, `agent.view.clear` |
...
`workspace.create` is federation-aware when `cwd` is omitted. Herdr resolves the workspace the create starts from — the sidebar-selected workspace while the TUI is in navigate mode, otherwise the active workspace (a create confirmed from the name dialog stays pinned to the workspace focused when the dialog opened) — and if that workspace belongs to a live remote mount, the create is sent to the serving host instead of performed locally. This applies to every caller, including the CLI and scripts. The response is then `workspace_create_requested` with the `origin` mount key rather than `workspace_created`: it carries no workspace, tab, or pane ids, since the workspace does not exist yet; it materializes locally through that mount's normal resync, exactly like a workspace the remote user created. Passing an explicit `cwd` is always treated as a deliberate local-directory choice and is never redirected. When the source workspace's mount has no live link, the request fails with `remote_workspace_create_unsupported` instead of silently creating a local workspace.

`tab.create` is federation-aware in place; there is no separate `tab.create_remote` verb. When the target workspace — the explicit `workspace_id`, otherwise the active workspace — belongs to a live remote mount, the tab is created on the serving host instead of locally, for every caller including the CLI and scripts. The response is then `tab_create_requested` with the `origin` mount key rather than `tab_created`: it carries no tab or pane ids, since the tab does not exist yet; it appears locally through that mount's normal resync, exactly like a tab the remote user created. Scripted callers that branch on `tab_created` must handle this shape. The `label` travels to the serving host, where it is sanitized and length-clamped before it reaches that host's own chrome. An explicit `cwd` fails with `remote_tab_cwd_unsupported` and a non-empty `env` with `remote_tab_env_unsupported`: both are local-machine concepts with no meaning on the serving host, so they are refused rather than silently ignored. When the target workspace's mount has no live link, the request fails with `remote_tab_create_unsupported` instead of silently creating a local tab. Once `tab_create_requested` has been returned, the serving host's own answer arrives asynchronously and is never reported back to the API or CLI caller: a refusal — including `tab_create_redirected` when the serving host would itself redirect the create onto a host it mounts, `not the mounted controller`, and `<code>: tab could not be created on the remote host` — surfaces only as a TUI toast and a server log entry. Scripted callers must wait for the tab to appear through resync or events rather than expecting a second response.

`workspace.close` and `tab.close` only ever remove your local mirror of a federated (remote-mounted) workspace or tab; they never reach the federation link, so the copy on the serving host keeps running. `workspace.close_remote` and `tab.close_remote` are the distinct opt-in verbs that ask the serving host to close its own workspace or tab. Because that answer arrives asynchronously over the federation link, they cannot report the finished close: they succeed with `workspace_close_requested` / `tab_close_requested` (each carrying the `origin` mount the request went out over) as soon as the request has been sent, and the local mirror disappears later, once the host confirms it closed. They return `remote_close_unsupported` when the target is not federated, its mount has no live link, or the peer never agreed to the close capability.

`WorkspaceInfo` and `TabInfo` both include a `name_source` field explaining why the reported `label`/`name` is what it is: `override` (a name you set), `mirrored` (a name mirrored from a live remote mount, federation-scoped only), `agent_identity` (derived from the occupying agent's detected or hook-reported identity), `cwd` (derived from the working directory), or `ordinal` (a stable position number, the fallback when nothing else applies). `PaneInfo` does not carry `name_source`: its `label` field is a plain edit-affordance value (the rename dialog's prefill), not a resolved display name. `tab.rename` and `workspace.rename` currently require a non-empty string `label`; there is no way to clear an override back to an auto-derived name through the API today.
...
`layout.balance` rebalances every split ratio in a tab so each leaf pane gets
an equal area, weighted by leaf count rather than tree shape. It takes the
same target params as `layout.export` (omit both to target the active tab).
The response is `type: "layout_balance"` with the updated portable `layout`.
It is a no-op (unchanged layout, still a success response) while the target
tab is zoomed, since the real split ratios are inert until un-zoom.
Split ratios are not federated. Ratio-only layout changes (`layout.balance`,
`layout.set_split_ratio`, and the auto-resize-splits behavior) neither
trigger a federation resync nor propagate through one — a resync reconciles
workspaces, tabs, and panes, and does not apply layout ratios. A mounted
remote workspace's local mirror can therefore show stale ratios for the life
of the mount.

```json
{"id":"req_balance","method":"layout.balance","params":{"tab_id":"w1:t1"}}
```

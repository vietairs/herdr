...
Linked plugins persist across restarts. Herdr writes a `plugins.json` registry file alongside `session.json` on every `plugin.link` and `plugin.unlink`. On startup, Herdr re-reads each manifest from its original path; if the file is missing or unparseable the entry is kept with a `warnings` field so `plugin.list` surfaces it.
...
Declare `platforms` at the top level with the OS identifiers (`linux`, `macos`, `windows`) your plugin supports. Omitting `platforms` is allowed for local development — `plugin.link` succeeds but the response includes a warning. Individual actions and events can declare their own `platforms` to override the plugin-level list; if omitted they inherit from the plugin. Invoking an action whose effective platforms do not include the current OS returns a `platform_unsupported` error.
...
`plugin.action.invoke` resolves the manifest action and returns the
Herdr-built invocation context. It does not execute the command yet.
Missing context fields are filled from the active workspace, tab, focused
pane, worktree provenance, and request id. Invoking an action from a

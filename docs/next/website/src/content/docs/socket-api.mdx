...
`pane_id`, `label`, `cwd`, `foreground_cwd`, and argv `command`. Split nodes use
...
The plugin API is an early host surface for local workflow tools. A plugin can be a local executable package with a `herdr-plugin.toml` manifest, or a process that registers actions directly. A plugin process can register actions, store namespaced JSON records, and ask Herdr to open a managed terminal pane for its UI.
...
The path can be a plugin directory containing `herdr-plugin.toml` or a direct manifest path. The manifest shape for this slice is:
...
In this early slice, linking validates and records manifest metadata. Manifest actions and event hooks are not executed until the runner work lands.

Register an action:

```json
{"id":"req_plugin_action","method":"plugin.action.register","params":{"plugin_id":"example.issue-flow","action_id":"assign-issue","title":"Assign Issue","contexts":["workspace","pane"],"entrypoint":"assign"}}
```

Invoke an action:

```json
{"id":"req_plugin_invoke","method":"plugin.action.invoke","params":{"plugin_id":"example.issue-flow","action_id":"assign-issue","context":{"invocation_source":"keybinding"}}}
```

In this early slice, `plugin.action.invoke` resolves the registered action and
returns the Herdr-built invocation context. It does not launch the action
entrypoint yet. Missing context fields are filled from the active workspace,
tab, focused pane, worktree provenance, and request id.

Store a workspace-scoped JSON value:

```json
{"id":"req_plugin_store","method":"plugin.storage.set","params":{"plugin_id":"example.notes","scope":"workspace","workspace_id":"1","key":"pins","value":[{"text":"follow up"}]}}
```

Open a managed terminal UI:

```json
{"id":"req_plugin_pane","method":"plugin.pane.open","params":{"plugin_id":"example.board","entrypoint":"board","placement":"zoomed","target_pane_id":"1-1","argv":["herdr-board","--workspace","1"],"focus":true}}
```

`plugin.pane.open` launches `argv` as an argv-backed terminal pane. `placement` can be `split`, `tab`, or `zoomed`. The pane behaves like a normal Herdr pane, but `plugin.pane.focus` and `plugin.pane.close` only operate on panes opened through the plugin API.

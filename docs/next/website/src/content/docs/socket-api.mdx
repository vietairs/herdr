...
The plugin API is an early host surface for local workflow tools. A plugin is a local executable package with a `herdr-plugin.toml` manifest. Actions are manifest-only; runtime action registration is not part of v1.
...
Herdr injects `HERDR_SOCKET_PATH`, `HERDR_ENV=1`, `HERDR_PLUGIN_ID`,
`HERDR_PLUGIN_CONTEXT_JSON`, and available `HERDR_WORKSPACE_ID`,
`HERDR_TAB_ID`, and `HERDR_PANE_ID` values. Action commands also receive
...
terminal pane. `placement` defaults to `overlay`, and can be overridden with
`split`, `tab`, or `zoomed`. Overlay panes target the active pane. Split and
zoomed panes target an existing pane; tab panes can target a workspace. The pane
